...
# Das tranierte Modell und die Daten müssen separat runter geladen werden, und bei ./dataset gespeichert werden
...
def get_recommendations():
    ...
    for v in scores.values():
        ...
        decisiveWord = getDecisiveWords(scores, a, index)

        paper['authors'] = authors
        paper['decisive_words'] = decisiveWord
...
def getPaperData(paperid, cur):
    ...
    query = 'SELECT originaltitle, publishedyear, referencecount, citationcount FROM papers where paperid='+str(paperid)
    cur.execute(query)
    row = cur.fetchone()

    return {'paperid': paperid, 'title': row['originaltitle'], 'url':url, 'year': row['publishedyear'], 'referencecount': row['referencecount'], 'citationcount': row['citationcount']}
